...
import com.zipple.module.mainpage.domain.ReviewPageResponse;
...
public class MainPageController {
    ...
    @Operation(summary = "공인 중개사 매칭 전체 또는 카테고리 조회", description = "페이징을 이용하여 중개사의 매칭 프로필을 조회합니다.")
    ...
    public ResponseEntity<ReviewPageResponse> getAgentReview(
            @Parameter(name = "agentId", description = "중개사 ID")
            @PathVariable(value = "agentId") String agentId,
            @Parameter(name = "page", description = "페이지 번호", example = "0")
            @RequestParam(value = "page", defaultValue = "0") Integer page,
        ...
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        ReviewPageResponse reviews = mainPageService.getAgentReview(agentId, pageable);
